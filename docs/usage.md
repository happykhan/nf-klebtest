# nf-core/klebtest: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/klebtest/usage](https://nf-co.re/klebtest/usage)
...
```console
nextflow run nf-core/klebtest --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/klebtest
```
...
First, go to the [nf-core/klebtest releases page](https://github.com/nf-core/klebtest/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
